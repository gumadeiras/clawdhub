...
- Start the gateway. Telegram starts when a token is resolved (env or config).
...
- Run clawdbot logs --follow and look for from.id.
...
- Tool: telegram with sendMessage action (to, content, optional mediaUrl, replyToMessageId, messageThreadId, buttons).
...
[Suggest edits](https://github.com/clawdbot/clawdbot/edit/main/docs/channels/telegram.mdx)

[Raise issue](https://github.com/clawdbot/clawdbot/issues/new?title=Issue on docs&body=Path: /channels/telegram)
